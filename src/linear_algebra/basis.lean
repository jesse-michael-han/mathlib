...
disjoint (lc.supported α β v s) (lc.total ι α β v).ker
variables {α} {β} {v}

theorem linear_independent_iff : linear_independent α β v s ↔
  ∀l ∈ lc.supported α β v s, lc.total ι α β v l = 0 → l = 0 :=
...
lemma linear_independent.unique (hs : linear_independent α β v s) {l₁ l₂ : lc ι α β v} :
  l₁ ∈ lc.supported α β v s → l₂ ∈ lc.supported α β v s →
...
λ h, ne $ eq.symm begin
  suffices : (finsupp.single i 1 : lc ι α β v) i = 0, {simpa},
  rw disjoint_def.1 hs _ (lc.single_mem_supported β v 1 h),
  {refl}, {simp [hi]}
end
...
begin
  ...
  have : lc.total ι α β v ls ∈ map (lc.total ι α β v) (lc.supported α β v t),
...
lemma linear_independent.injective (zero_ne_one : (0 : α) ≠ 1)
  ...
  (hij: v i = v j) : i = j :=
begin
  let l : lc ι α β v := finsupp.single i (1 : α) - finsupp.single j 1,
  have h_supp : l ∈ lc.supported α β v s,
  { dsimp only [l],
    rw [lc.mem_supported],
    intros k hk,
...
def linear_independent.total_equiv : lc.supported α β v s ≃ₗ span α (v '' s) :=
linear_equiv.of_bijective (lc.total_on α β v s)
  (linear_independent_iff_total_on.1 hs) (lc.total_on_range _)

def linear_independent.repr : span α (v '' s) →ₗ[α] lc ι α β v :=
(submodule.subtype _).comp (hs.total_equiv.symm : span α (v '' s) →ₗ[α] lc.supported α β v s)
...
lemma linear_independent.repr_range : hs.repr.range = lc.supported α β v s :=
by rw [linear_independent.repr, linear_map.range_comp,
       linear_equiv.range, map_top, range_subtype]

lemma linear_independent.repr_eq {l : lc ι α β v} (h : l ∈ lc.supported α β v s) {x} (eq : lc.total ι α β v l = ↑x) : hs.repr x = l :=
by rw ← (subtype.eq' eq : (lc.total_on α β v s : lc.supported α β v s →ₗ span α (v '' s)) ⟨l, h⟩ = x);
...
begin
  apply hs.repr_eq (lc.single_mem_supported _ _ _ hi),
  simp [lc.total_single, hx]
end

lemma linear_independent.repr_supported (x) : hs.repr x ∈ lc.supported α β v s :=
((hs.total_equiv.symm : span α (v '' s) →ₗ[α] lc.supported α β v s) x).2
...
⟨λ hs i hi a ha, begin
  ...
  have := (lc.supported α β v s).sub_mem
    (lc.supported_mono (diff_subset _ _) hl) (lc.single_mem_supported _ v a hi),
  ...
  { have : finsupp.single x (l x) - l ∈ lc.supported α β v s :=
      sub_mem _ (lc.single_mem_supported _ _ _ xs) ls,
